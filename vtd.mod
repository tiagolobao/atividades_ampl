...
var x{ (s,d) in LIGHTPATHS, (i,j) in ENLACES } binary; # fluxo do par s-d que passa pelo enlace  i-j (ANTES DO DPI)
var y{ (s,d) in LIGHTPATHS, (i,j) in ENLACES } binary; # fluxo do par s-d que passa pelo enlace i-j (DEPOIS DO DPI)
...
param vCPU{ i in V } >= 0; # quantidade de vCPU ativos
...
sum{ (i,j) in ENLACES: i=d  }(y[s,d,i,j] + dpi[s,d,i]) = 1;

subject to conserv_y {(s,d) in LIGHTPATHS}:
sum{ (i,j) in ENLACES: i!=d }(y[s,d,i,j] + dpi[s,d,i]) = sum{ (i,j) in LIGHTPATHS: i!=s }y[s,d,j,i];
...
sum{ i in V }( fs*dpi[s,d,i] ) <= Cdpi*vCPU[1];
